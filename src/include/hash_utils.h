...
inline unsigned long hash(const string &s) {unsigned long retval=0; for (unsigned long i=0; i<s.size(); ++i) retval=retval*33+(static_cast<unsigned char>(s[i])&127); return retval;}

template<typename T>
inline unsigned long hash(const T& t) { return t.hash(); }

template<typename T1, typename T2>
inline unsigned long hash(const pair<T1, T2> &o) { return hash(o.first)*33 + hash(o.second) ; }
