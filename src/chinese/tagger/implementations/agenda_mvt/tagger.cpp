...
//      std::cout << m_weights->m_rules.canSeparate(i) << std::endl;
   ...
   for ( unsigned i=0; i<correct->size(); ++i ) {
      ...
      if ( PENN_TAG_CLOSED[tag] || tag==PENN_TAG_M ) {
      ...
      if ( index > 0 ) {
         ...
         for (j=0; j<m_Agenda.generatorSize(); ++j) {
            ...
            if ( ( m_weights->m_rules.canAppend(index) ) && // ( index > 0 ) &&
                 ( pGenerator->getWordStart(pGenerator->size()-1)<last_nosep || pGenerator->getTag(pGenerator->size()-1).code() == PENN_TAG_M || pGenerator->getWordLength(pGenerator->size()-1) < m_weights->m_maxLengthByTag[pGenerator->getTag(pGenerator->size()-1).code()] )
               ) {
