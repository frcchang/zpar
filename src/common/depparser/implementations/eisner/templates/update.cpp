...
#define updateBigrams(x) \
   ...
   cast_weights->m_mapHeadTagDepTag[ make_pair((head_tag.code()<<CTag::SIZE)+dep_tag.code(),x) ].updateCurrent( amount , round ) ;
...
#define updateSurrounding(x) \
   ...
   if (nc_ll) cast_weights->m_mapSurroundingTagsLL[ make_pair(encodeTags(CTag(CTag::NONE), head_tag_l, dep_tag, dep_tag_l), x) ].updateCurrent( amount , round ) ; \
   if (nc_lr) cast_weights->m_mapSurroundingTagsLR[ make_pair(encodeTags(CTag(CTag::NONE), head_tag_l, dep_tag, dep_tag_r), x) ].updateCurrent( amount , round ) ; \
   if (nc_rl) cast_weights->m_mapSurroundingTagsRL[ make_pair(encodeTags(CTag(CTag::NONE), head_tag_r, dep_tag, dep_tag_l), x) ].updateCurrent( amount , round ) ; \
   if (nc_rr) cast_weights->m_mapSurroundingTagsRR[ make_pair(encodeTags(CTag(CTag::NONE), head_tag_r, dep_tag, dep_tag_r), x) ].updateCurrent( amount , round ) ; \
   if (nc_ll) cast_weights->m_mapSurroundingTagsLL[ make_pair(encodeTags(head_tag, head_tag_l, CTag(CTag::NONE), dep_tag_l), x) ].updateCurrent( amount , round ) ; \
   if (nc_lr) cast_weights->m_mapSurroundingTagsLR[ make_pair(encodeTags(head_tag, head_tag_l, CTag(CTag::NONE), dep_tag_r), x) ].updateCurrent( amount , round ) ; \
   if (nc_rl) cast_weights->m_mapSurroundingTagsRL[ make_pair(encodeTags(head_tag, head_tag_r, CTag(CTag::NONE), dep_tag_l), x) ].updateCurrent( amount , round ) ; \
   if (nc_rr) cast_weights->m_mapSurroundingTagsRR[ make_pair(encodeTags(head_tag, head_tag_r, CTag(CTag::NONE), dep_tag_r), x) ].updateCurrent( amount , round ) ; \
   if (nc__l) cast_weights->m_mapSurroundingTagsLL[ make_pair(encodeTags(head_tag, CTag(CTag::NONE), dep_tag, dep_tag_l), x) ].updateCurrent( amount , round ) ; \
   if (nc__r) cast_weights->m_mapSurroundingTagsLR[ make_pair(encodeTags(head_tag, CTag(CTag::NONE), dep_tag, dep_tag_r), x) ].updateCurrent( amount , round ) ; \
   if (nc_l_) cast_weights->m_mapSurroundingTagsLR[ make_pair(encodeTags(head_tag, head_tag_l, dep_tag, CTag(CTag::NONE)), x) ].updateCurrent( amount , round ) ; \
   if (nc_r_) cast_weights->m_mapSurroundingTagsRR[ make_pair(encodeTags(head_tag, head_tag_r, dep_tag, CTag(CTag::NONE)), x) ].updateCurrent( amount , round ) ;
