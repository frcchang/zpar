...
#define getSurroundingScoreTemplate(x) \
   ...
   if (nc_ll) retval += cast_weights->m_mapSurroundingTagsLL.find( make_pair(encodeTags(PENN_TAG_NONE, head_tag_l, dep_tag, dep_tag_l), x), g_zeroScore ).score(m_nScoreIndex) ; \
   if (nc_lr) retval += cast_weights->m_mapSurroundingTagsLR.find( make_pair(encodeTags(PENN_TAG_NONE, head_tag_l, dep_tag, dep_tag_r), x), g_zeroScore ).score(m_nScoreIndex) ; \
   if (nc_rl) retval += cast_weights->m_mapSurroundingTagsRL.find( make_pair(encodeTags(PENN_TAG_NONE, head_tag_r, dep_tag, dep_tag_l), x), g_zeroScore ).score(m_nScoreIndex) ; \
   if (nc_rr) retval += cast_weights->m_mapSurroundingTagsRR.find( make_pair(encodeTags(PENN_TAG_NONE, head_tag_r, dep_tag, dep_tag_r), x), g_zeroScore ).score(m_nScoreIndex) ; \
   if (nc_ll) retval += cast_weights->m_mapSurroundingTagsLL.find( make_pair(encodeTags(head_tag, head_tag_l, PENN_TAG_NONE, dep_tag_l), x), g_zeroScore ).score(m_nScoreIndex) ; \
   if (nc_lr) retval += cast_weights->m_mapSurroundingTagsLR.find( make_pair(encodeTags(head_tag, head_tag_l, PENN_TAG_NONE, dep_tag_r), x), g_zeroScore ).score(m_nScoreIndex) ; \
   if (nc_rl) retval += cast_weights->m_mapSurroundingTagsRL.find( make_pair(encodeTags(head_tag, head_tag_r, PENN_TAG_NONE, dep_tag_l), x), g_zeroScore ).score(m_nScoreIndex) ; \
   if (nc_rr) retval += cast_weights->m_mapSurroundingTagsRR.find( make_pair(encodeTags(head_tag, head_tag_r, PENN_TAG_NONE, dep_tag_r), x), g_zeroScore ).score(m_nScoreIndex) ; \
   if (nc__l) retval += cast_weights->m_mapSurroundingTagsLL.find( make_pair(encodeTags(head_tag, PENN_TAG_NONE, dep_tag, dep_tag_l), x), g_zeroScore ).score(m_nScoreIndex) ; \
   if (nc__r) retval += cast_weights->m_mapSurroundingTagsLR.find( make_pair(encodeTags(head_tag, PENN_TAG_NONE, dep_tag, dep_tag_r), x), g_zeroScore ).score(m_nScoreIndex) ; \
   if (nc_l_) retval += cast_weights->m_mapSurroundingTagsLR.find( make_pair(encodeTags(head_tag, head_tag_l, dep_tag, PENN_TAG_NONE), x), g_zeroScore ).score(m_nScoreIndex) ; \
   if (nc_r_) retval += cast_weights->m_mapSurroundingTagsRR.find( make_pair(encodeTags(head_tag, head_tag_r, dep_tag, PENN_TAG_NONE), x), g_zeroScore ).score(m_nScoreIndex) ; 
