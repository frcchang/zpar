...
#define getOrUpdateLabeledScoreTemplate(x)\
   ...
   retval += cast_weights->m_mapHeadTagLabel.getOrUpdateScore( std::make_pair(head_tag, x) , m_nScoreIndex , amount , round ) ;\
   retval += cast_weights->m_mapDepTagLabel.getOrUpdateScore( std::make_pair(dep_tag, x) , m_nScoreIndex , amount , round ) ;\
