...
void auto_train(std::string sOutputFile, std::string sFeatureFile, const std::string &sKnowledgeBase) {
   CTagger decoder(sFeatureFile, true);
   if (sKnowledgeBase.size())
      decoder.loadTagDictionary(sKnowledgeBase);
...
int main(int argc, char* argv[]) {
   try {
      ...
      for (int i=0; i<training_rounds; ++i)
         auto_train(argv[1], argv[2], sTagDict);
